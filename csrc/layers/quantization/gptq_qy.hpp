...
public:
    ...
    void convert_and_take_ownership(infinicore::Tensor &weight, infinicore::Tensor &zeros, infinicore::Tensor &scales,
                                    const infinicore::Tensor &g_idx, const infinicore::Device &target_device) {
        if (converted_) {
...
private:
    static inline std::vector<uint8_t> unpack_int32_to_nibbles_3d_(const infinicore::Tensor &packed, int bits) {
    ...
    static inline std::vector<float> unpack_zeros_to_fp32_2d_(const infinicore::Tensor &packed_zeros, int bits) {
        assert(bits == 4 || bits == 8);
        const int values_per_int32 = 32 / bits;
    ...
    static inline infinicore::Tensor make_tensor_from_host_(const void *data, size_t bytes,
